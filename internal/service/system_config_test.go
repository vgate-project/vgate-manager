...
func TestGetTelegramConfig_AdminChatIDs(t *testing.T) {
	seed := func(t *testing.T, value string) *SystemConfigService {
		db := cfgDB(t)
		if value != "" {
			if err := db.Create(&model.SystemConfig{Key: CfgKeyTelegramAdminChatIDs, Value: value}).Error; err != nil {
				t.Fatalf("seed: %v", err)
			}
		}
		return NewSystemConfigService(db)
	...
	// The admin UI's tags input emits a JSON array of numeric strings; this
	// must resolve to the equivalent []int64 (group/channel IDs are negative).
	svc := seed(t, `["123", "-456"]`)
	cfg, err := svc.GetTelegramConfig()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(cfg.AdminChatIDs) != 2 || cfg.AdminChatIDs[0] != 123 || cfg.AdminChatIDs[1] != -456 {
		t.Fatalf("string-array form: got %v, want [123 -456]", cfg.AdminChatIDs)
	}

	// A hand-edited config may use a JSON array of numbers — still accepted.
	svc = seed(t, `[123, -456]`)
	cfg, err = svc.GetTelegramConfig()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if len(cfg.AdminChatIDs) != 2 || cfg.AdminChatIDs[0] != 123 || cfg.AdminChatIDs[1] != -456 {
		t.Fatalf("number-array form: got %v, want [123 -456]", cfg.AdminChatIDs)
	}
	...
	if len(cfg.AdminChatIDs) != 2 || cfg.AdminChatIDs[0] != 789 || cfg.AdminChatIDs[1] != -12 {
		t.Fatalf("whitespace form: got %v, want [789 -12]", cfg.AdminChatIDs)
	}

	// A non-numeric entry is skipped; valid entries are kept.
	svc = seed(t, `["123", "not-a-number", "-456"]`)
	cfg, err = svc.GetTelegramConfig()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(cfg.AdminChatIDs) != 2 || cfg.AdminChatIDs[0] != 123 || cfg.AdminChatIDs[1] != -456 {
		t.Fatalf("mixed form: got %v, want [123 -456]", cfg.AdminChatIDs)
	}

	// Empty array / absent key ⇒ empty slice, no error (bot stays off).
	svc = seed(t, `[]`)
	cfg, err = svc.GetTelegramConfig()
	if err != nil || len(cfg.AdminChatIDs) != 0 {
		t.Fatalf("empty array: got %v err %v, want empty", cfg.AdminChatIDs, err)
	}
	svc = seed(t, "")
	cfg, err = svc.GetTelegramConfig()
	if err != nil || len(cfg.AdminChatIDs) != 0 {
		t.Fatalf("absent key: got %v err %v, want empty", cfg.AdminChatIDs, err)
	}

	// An unparseable (non-array) value is ignored and surfaces a warning, but
	// does not crash or produce partial IDs.
	svc = seed(t, `"123"`)
	cfg, err = svc.GetTelegramConfig()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if len(cfg.AdminChatIDs) != 0 {
		t.Fatalf("non-array value: got %v, want empty (ignored)", cfg.AdminChatIDs)
	}
}
